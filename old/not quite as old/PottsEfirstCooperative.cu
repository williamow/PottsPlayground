    ...
    )
{
    ...
    for (int iter = MinIter; iter < MaxIter; iter++){
        ...
        //split these out into separate loops so that memory accesses are more efficient
        for (int NHPP = thread_index; NHPP < nNHPPs; NHPP += coopSize)
        ...
        for (int NHPP = thread_index; NHPP < nNHPPs; NHPP += coopSize){
            ...
            float NhppRate = exp2(-proposed_dE/T);
            float NewPriority = -log2(RngUniform())/NhppRate; 
