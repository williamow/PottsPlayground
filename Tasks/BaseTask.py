...
class BaseTask:
	...
	def SetPartitions(self, qSizes):
		...
		"""
		if type(qSizes) == list:
			self.qSizes = numpy.zeros(len(qSizes), dtype='int32')
		else:
	...
	def constTemp(self, niters, temp):
	...
	def LinearTemp(self, niters, temp, t2=0.0001):
		PwlTemp = numpy.zeros([2, 2], dtype="float32")
		PwlTemp[0,0] = temp
		PwlTemp[0,1] = t2
		PwlTemp[1,0] = 0
	...
	def constTempLog2Space(self, temp, nReports, nIters):
		#niters is implicitly equal to 10*(2**nReports)
	...
	def SawtoothTempLog2Space(self, MaxTemp, MinTemp, nTeeth, nIters, FirstTooth=10):
		...
		for i in range(nTeeth):
			...
			PwlTemp[0,i*2+2] = MaxTemp #top of next tooth
