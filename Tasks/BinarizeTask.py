...
class BinarizeTask(BaseTask.BaseTask):
	"""
		...
		for i in range(nUnits): #iterates through rows of weight matrix
			...
			# for kspec in PottsTask.kMapLists[i]: #iterates through sparse format of column kernels
				...
				for x in range(sz_x): #iterate through all the values in the kernel:
					for y in range(sz_y):
						self.AddKernel(lambda n: self.IsingKernel(n), pos_x+x, pos_y+y, weight=k[x,y])
			...
			for m1 in range(q):
				...
				for m2 in range(q):
					if m1 == m2:
						...
						self.AddKernel(lambda n: self.IsingKernel(n), m10+m1, m10+m2, weight=w_inhib)

		self.CompileKernels()


	def IsingKernel(self, n=False):
		if n:
			return "ik"
		k = numpy.zeros([2,2])
		k[1,1] = 1
	...
	def IsingToPottsSemantics(self, IsingState):
		...
		return numpy.array(PottsState, dtype='int')
